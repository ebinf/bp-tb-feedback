...
<svelte:head>
	<title>Stimmungsbild {data.group.number}: {data.group.name}</title>
	<meta property="og:site_name" content={PUBLIC_SITE_NAME} />
	<meta property="og:locale" content="de_DE" />
	<meta property="og:title" content="Stimmungsbild {data.group.number}: {data.group.name}" />
	<meta property="og:type" content="website" />
	<meta property="og:url" content="{PUBLIC_BASE_URL}/poll/{data.link}" />
	<meta property="og:description" content="{data.question} Nimm am Stimmungsbild teil!" />
</svelte:head>
...
<main>
	...
	<div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
		...
		>
			...
			>
				<h2 class="mb-4 text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:tracking-tight">
					{data.question}
				</h2>
				...
				{:else if !data.open}
					<div class="mb-6">
						<Toast dismissable={false} style="fail"
							>Dieses Stimmungsbild ist leider bereits geschlossen. Falls du noch abstimmen
							möchtest, bitte deine Teambegleitung es wieder zu öffnen.</Toast
						>
					</div>
				{/if}

				<Form disabled={!!form?.success || !data.open} />
			</div>
