...
<Header>
	...
	<span slot="description"
		>Hier erscheint das Feedback, das die Gruppe dir über ihren Link gegeben hat. {#if data.group?.term.active}Teile
			den Link mit der Gruppe, um Feedback zu erhalten.{/if}</span
	>
	<span slot="buttons">
...
{#if data.feedback?.length}
	...
	></Table>
{:else}
	<Empty link_created={!!data.group?.feedback_link?.length} active={data.group?.term.active} />
{/if}
